...
class Pos extends CI_Controller
{
    ...
    public function view_invoice()
    {
        ...
        $data['lang_cust_address'] = $this->lang->line('address');
        $data['lang_products'] = $this->lang->line('products');
        $data['lang_qty'] = $this->lang->line('qty');
        $data['lang_per_item'] = $this->lang->line('per_item');
        $data['lang_total'] = $this->lang->line('total');
    ...
    public function insertSale()
    {
        if (isset($_POST['hold_bill_submit'])) {
            ...
            $rowucount = $this->input->post('row_count');
		...
		<?php
            ...
            $fromcash = $this->input->post('fromcash');
            $frompaid = $this->input->post('paid');
			
			
            $paid_amt = $fromcash + $frompaid;
            ...
            $cust_address = $custDtaData[0]->address;
            ...
            $vt_status = '';
            if ($paid_amt < $grandTotal) {          // Debit;
                $vt_status = '0';
            } if($paid_amt >= $grandTotal ) {        // Full Payment;
                   $vt_status = '1';
            }
    ...
    // Insert debit_cradit table
            $ins_debit_cradit_data = array(
                'cust_id' => $customer_id,
                'debit_amt' =>  $paid_amt,
                'outlet_id' => $user_outlet,
                'cradit_amt' => $grandTotal,
                'pay_method' => $pay_name,
                 'created_user_id' => $user_id,
            );
            
         $this->Constant_model->insertData('debit_cradit', $ins_debit_cradit_data);
...
<div id="wrapper">
    ...
    public function addNewCustomer()
    {
        ...
        $address = strip_tags($this->input->get('address'));
    ...
    public function loadTodaySales()
    {
        ...
        $orderResult = $this->db->query("SELECT * FROM orders WHERE ordered_datetime >= '$today_start' AND ordered_datetime <= '$today_end' AND outlet_id = '$outlet_id' ");
        ...
        for ($i = 0; $i < count($orderData); ++$i) {
            ...
            if ($payment_method == '1') {
                $total_cash += $grandTotal;

                $total_amt += $grandTotal;
            }
            if ($payment_method == '2') {
                $total_nett += $grandTotal;

                $total_amt += $grandTotal;
            }
            if ($payment_method == '3') {
                $total_visa += $grandTotal;

                $total_amt += $grandTotal;
            }
            if ($payment_method == '4') {
                $total_master += $grandTotal;

                $total_amt += $grandTotal;
            }
            if ($payment_method == '5') {
                $total_cheque += $grandTotal;
